...
<aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse w-auto h-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            <?php foreach ($menusBar as $menuBar):
                $collapse = '';
                $route = '';
                ...
                <li class="nav-item">
                    <a href="{{ $route }}" data-bs-toggle="{{ $collapse }}" role="button" aria-expanded="false"
                        class="nav-link {{ $menuBar['activeMenu'] }}">
                    ...
                    <?php if (isset($menuBar['collapse'])): ?>
                        <div class="collapse {{ $menuBar['collapse']['show'] }}" id="{{ $menuBar['collapse']['id'] }}">
                            <ul class="nav ms-4">
                                <?php foreach ($menuBar['collapses'] as $itemCollapse): ?>
                                    <li class="nav-item {{ $itemCollapse['activeMenuItem'] }}">
                                        <a class="nav-link {{ $itemCollapse['activeMenuItem'] }}"
                                            href="{{ route($itemCollapse['route']) }}">
